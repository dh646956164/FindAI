...
export default async function NewsPage() {
  ...
  return <>
    ...
    <div className="container grid gap-8 py-12 lg:grid-cols-[1fr_280px]">
      ...
      <aside><div className="sticky top-24 rounded-2xl bg-slate-950 p-6 text-white shadow-xl"><Newspaper className="size-7 text-indigo-300"/><h2 className="mt-4 text-lg font-black">情报收录范围</h2><ul className="mt-4 space-y-3 text-sm text-slate-400">{["AI 工具新上线","模型版本更新","Agent 新产品","API 价格与额度","重要功能变化","行业与高校应用"].map((item)=><li key={item}>· {item}</li>)}</ul><p className="mt-6 border-t border-white/10 pt-5 text-xs leading-6 text-slate-500">每天自动检查 24 个精选工具的官方页面与公开动态。自动聚合只提示变化，推荐结论仍由人工规则保守维护。</p></div></aside>
    </div>
  </>;
}
