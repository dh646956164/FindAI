...
import { compareNationalUsage } from "@/lib/curated";
...
export function ToolsClient({ tools: allTools }: { tools: ToolView[] }) {
  ...
  }).sort((a, b) => filters.sort === "name" ? a.name.localeCompare(b.name, "zh-CN") : filters.sort === "updated" ? +new Date(b.updatedAt) - +new Date(a.updatedAt) : compareNationalUsage(a, b));
  ...
  return <>
    <section className="border-b bg-white"><div className="container py-10"><Badge className="mb-3"><SlidersHorizontal className="mr-1.5 size-3" />{allTools.length} 个精选工具</Badge><h1 className="text-balance text-3xl font-black tracking-tight md:text-4xl">任务工具箱</h1><p className="mt-3 max-w-3xl leading-7 text-muted-foreground">汇聚24个日常使用的AI大模型，提升你的学习工作效率！</p>
